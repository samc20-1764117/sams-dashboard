...
function _vidOvAllProgRow(v){
  ...
  // Big row now DARK (bold var(--text)), matching how Big rows read in the Up Next list
  // (_vidOvMenuItem) — was flattened to plain var(--muted)/500 on 2026-09-14 to match Up Next's
  // OLD muted treatment at the time, but Up Next's own Big-row style has since become dark and
  // this "In Progress" column never followed, leaving the two visibly inconsistent (2026-09-22
  // feedback: "i want the big videos to have the darker text to match how they look in up
  // next"). Keep these two in sync going forward — if Up Next's Big-row style changes again,
  // mirror it here too.
