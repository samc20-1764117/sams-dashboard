...
function renderPupSkillsHighlight(){
  ...
  wrap.style.cssText='display:flex;gap:5px;margin:5px;flex-shrink:0';
...
function tRowExtra(t){
  ...
  return`<div class="ti ti-${sl}${_bdDone?' done':''}" style="background:${s.bg}${_bdDone?';opacity:.45':''}" id="ti-${t.id}" ${bdDrag} onclick="selTask(event,'${t.id}')">
    ${isTv?`<button class="pack-icon-btn" onclick="event.stopPropagation();openPackingModal('${t._srcId}')" title="Packing list" style="opacity:.6;width:12px;min-width:12px;justify-content:center;padding:1px 0;margin:0">${_PACK_SVG}</button>`:''}
