...
function _vidOvMenuItem(v,steps,focusSet){
  ...
  const _addBtn=`<button onclick="event.stopPropagation();_vidOvInlineAdd('${sid}',null,null,this.closest('[data-vidrow]'))" style="font-size:9px;font-weight:700;width:13px;height:13px;display:inline-flex;align-items:center;justify-content:center;border-radius:2px;border:1px solid ${_onCal?'var(--accent)':'var(--border)'};background:var(--bg);color:${_onCal?'var(--accent)':'var(--muted)'};cursor:pointer;padding:0;flex-shrink:0;box-sizing:border-box" title="Add small video">+</button>`;
  ...
  const _countBadge=_smallCount?`<span style="font-size:10px;font-weight:500;color:var(--muted);margin-left:2px">· ${_smallCount}</span>`:'';
