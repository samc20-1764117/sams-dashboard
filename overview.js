...
function _vidOvMenuItem(v,steps,focusSet){
  ...
  const _addBtn=`<button onclick="event.stopPropagation();_vidOvInlineAdd('${sid}',null,null,this.closest('[data-vidrow]'))" style="font-size:11px;font-weight:400;width:13px;height:13px;display:flex;align-items:center;justify-content:center;line-height:1;border-radius:2px;border:1px solid ${_onCal?'var(--accent)':'var(--border)'};background:var(--bg);color:${_onCal?'var(--accent)':'var(--muted)'};cursor:pointer;padding:0 0 1px 0;flex-shrink:0;box-sizing:border-box" title="Add small video">+</button>`;
