...
function _vidStepAssignToDay(vidId,step,ds){
  ...
  if(prevDs&&prevDs!==ds){
    (st.blocks||[]).filter(bl=>String(bl._vidStepVid)===String(vidId)&&bl._vidStepName===step&&bl.ds===prevDs).forEach(bl=>{
      bl.ds=ds;sbUpdateBlock(bl.id,{ds});
    });
  }
  save();renderAll();if(document.getElementById('tbGrid'))renderDayTB();
  const panel=document.getElementById('vidOvPanel');if(panel&&panel.style.display==='block')_renderVidOvMenu();
  pushUndo(()=>{const m2=_vidStepDayMap();if(prev)m2[key]=prev;else delete m2[key];_vidStepDayMapSet(m2);if(prevDs&&prevDs!==ds){(st.blocks||[]).filter(bl=>String(bl._vidStepVid)===String(vidId)&&bl._vidStepName===step&&bl.ds===ds).forEach(bl=>{bl.ds=prevDs;sbUpdateBlock(bl.id,{ds:prevDs});});}save();renderAll();if(document.getElementById('tbGrid'))renderDayTB();const p2=document.getElementById('vidOvPanel');if(p2&&p2.style.display==='block')_renderVidOvMenu();},'Moved step');
}
...
function _renderVidOvMenu(){
  ...
  html+='<span style="width:14px;flex-shrink:0;margin-left:6px"></span>';
  html+='</div>';
...
function _vidOvMenuItem(v,steps,focusSet){
  ...
  const _postField=`<div class="vid-ov-post" style="width:28px;flex-shrink:0;display:flex;align-items:center;justify-content:flex-end;align-self:stretch;cursor:pointer;margin:-5px -4px -5px 0;padding:5px 4px 5px 8px" onmousedown="event.stopPropagation();event.preventDefault();_vidOvEditPostDate('${sid}',this)" ondblclick="event.stopPropagation()" title="${v.post_date||'Set post date'}"><span style="font-size:9px;font-variant-numeric:tabular-nums;font-family:system-ui,-apple-system,sans-serif;color:${_postColor};line-height:1">${_postDate||''}</span></div>`;
  const _focusCls=_isFocused?' vid-ov-focus':'';
  let html=`<div data-vidrow="${sid}" ${_dragAttr} ${_dblAttr} ${_ctxAttr} ${_hov} class="${_focusCls}" style="padding:5px 19px 5px 6px;border-radius:6px;font-size:13px;font-weight:600;color:var(--text);cursor:grab;display:flex;align-items:center;gap:3px;transition:background .1s">${_addBtn}<span style="flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${escHtml(v.topic||v.title)}</span><div style="display:flex;gap:0;flex-shrink:0;align-items:center;margin-right:2px">${_vidOvStepDots(v,steps)}</div>${_postField}<div class="vid-ov-pctx" style="width:14px;flex-shrink:0;text-align:right;position:relative;margin-left:6px;align-self:center"><span class="vid-ov-pct" style="font-size:9px;opacity:.5;font-variant-numeric:tabular-nums;font-family:system-ui,-apple-system,sans-serif">${_vidOvPct(v,steps)?_vidOvPct(v,steps)+'%':''}</span>${_xBtn}</div></div>`;
  // Children (S/L videos)
...
function _vidOvEditPostDate(vidId,el){
  ...
  inp.style.cssText='width:28px;height:14px;font-size:9px;padding:0 1px;border:1px solid var(--accent);border-radius:3px;background:var(--bg);outline:none;color:var(--text);text-align:right;font-variant-numeric:tabular-nums;font-family:system-ui,-apple-system,sans-serif;box-sizing:border-box;margin:0;line-height:1';
  const origHtml=el.innerHTML;
  const _restoreHtml=(txt)=>{const color=v.post_date?_vidOvPostColor(v):'var(--muted)';el.innerHTML=`<span style="font-size:9px;font-variant-numeric:tabular-nums;font-family:system-ui,-apple-system,sans-serif;color:${color};line-height:1">${txt||''}</span>`;};
