...
function _renderVidOvMenu(){
  ...
  html+='<div style="display:flex;align-items:center;padding:2px 19px 2px 6px;gap:5px">';
  html+='<div style="width:13px;flex-shrink:0"></div>';
...
function _vidOvMenuItem(v,steps,focusSet){
  ...
  const _addBtn=`<div onclick="event.stopPropagation();_vidOvInlineAdd('${sid}',null,null,this.closest('[data-vidrow]'))" style="font-size:11px;font-weight:400;width:13px;height:13px;display:flex;align-items:center;justify-content:center;line-height:13px;border-radius:2px;border:1px solid ${_onCal?'var(--accent)':'var(--border)'};background:var(--bg);color:${_onCal?'var(--accent)':'var(--muted)'};cursor:pointer;flex-shrink:0;box-sizing:border-box;text-align:center" title="Add small video">+</div>`;
  ...
  const _countBadge=_smallCount?`<span style="font-weight:400;color:var(--muted);margin-left:3px">· ${_smallCount}</span>`:'';
  let html=`<div data-vidrow="${sid}" ${_dragAttr} ${_dblAttr} ${_ctxAttr} ${_hov} class="${_focusCls}" style="padding:3px 19px 3px 6px;border-radius:6px;font-size:12px;font-weight:600;color:var(--text);cursor:grab;display:flex;align-items:center;gap:5px;transition:background .1s">${_addBtn}<span style="flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;line-height:13px">${escHtml(v.topic||v.title)}${_countBadge}</span><div style="display:flex;gap:0;flex-shrink:0;align-items:center">${_vidOvStepDots(v,steps)}</div>${_postField}<div class="vid-ov-pctx" style="width:14px;flex-shrink:0;text-align:center;position:relative;margin-left:12px;display:flex;align-items:center;justify-content:center;line-height:12px"><span class="vid-ov-pct" style="font-size:9px;opacity:.5;font-variant-numeric:tabular-nums;font-family:system-ui,-apple-system,sans-serif;line-height:12px">${_vidOvPct(v,steps)?_vidOvPct(v,steps)+'%':''}</span>${_xBtn}</div></div>`;
  // Children (S/L videos)
...
function _vidOvInlineAdd(bigId,orderBefore,orderAfter,afterEl){
  ...
  row.innerHTML=`<div style="width:13px;flex-shrink:0;box-sizing:border-box;border:1px solid transparent;display:inline-flex;align-items:center;justify-content:center;color:rgba(140,135,160,.4);font-size:9px">└</div><input type="text" placeholder="Video topic..." style="flex:1;min-width:0;border:none;outline:none;background:transparent;font-size:11px;color:var(--text);font-weight:500;padding:3px 4px;border-radius:4px;box-shadow:inset 0 0 0 1px rgba(14,165,233,.3)">`;
