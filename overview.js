...
function renderPupSkillsHighlight(){
  ...
  wrap.style.cssText='display:flex;gap:7px;margin:7px 0 7px 11px;flex-shrink:0';
...
// ── Habits Highlight (Overview) ─────────────────────────────────────────────
// Shows only stage==='building' habits — the 1-3 you're actively working into
// routine right now. Established/future habits live entirely on the Habits page.
function renderHabitsHighlight(){
  const el=document.getElementById('habitsHighlight');if(!el)return;
  const ds=d2s(getDayDate(dayOff));
  const building=st.habits.filter(h=>h.stage==='building'&&h.is_enabled!==false)
    .sort((a,b)=>(a.sort_order??0)-(b.sort_order??0)||(a.name||'').localeCompare(b.name||''));
  if(!building.length){el.innerHTML='';el.style.cssText='display:none';return;}
  ...
  el.innerHTML=`<div style="display:flex;align-items:center;padding:4px 10px 2px;gap:6px;border-top:1px solid ${_dk()?'rgba(255,255,255,.04)':'rgba(0,0,0,.06)'};margin-top:2px">
    <span style="font-size:10px;font-weight:600;letter-spacing:.05em;color:var(--muted);text-transform:uppercase;flex:1">Building</span>
    <span style="font-size:10px;color:var(--muted);font-weight:500">${building.filter(h=>_habitDoneOn(h.id,ds)).length}/${building.length}</span>
    <button class="btn btn-ghost btn-xs" onclick="showPage('habits')" style="padding:1px 6px;font-size:11px;line-height:1.4" title="Open Habits page">↗</button>
  </div><div style="padding:0 0 4px">${rows}</div>`;
}
function _habitDoneOn(habitId,ds){return st.habitLogs.some(l=>String(l.habit_id)===String(habitId)&&l.date===ds);}
function _habitStreak(habitId,fromDs){
  const dates=new Set(st.habitLogs.filter(l=>String(l.habit_id)===String(habitId)).map(l=>l.date));
  let n=0,cur=new Date(fromDs+'T12:00:00');
  while(dates.has(d2s(cur))){n++;cur.setDate(cur.getDate()-1);}
  return n;
}
async function togHabitDone(habitId,done,ds){
  const wasDone=_habitDoneOn(habitId,ds);
  if(done&&!wasDone){
    const localId='l-'+Date.now();
    st.habitLogs.push({id:localId,habit_id:habitId,date:ds});
    save();renderHabitsHighlight();if(typeof renderHabitsPage==='function'&&activePg==='habits')renderHabitsPage();
    const res=await sbReqSilent('POST','habit_logs',{habit_id:habitId,date:ds},'');
    if(res&&res[0]){const idx=st.habitLogs.findIndex(l=>l.id===localId);if(idx>=0)st.habitLogs[idx]=res[0];save();}
    pushUndo(()=>{st.habitLogs=st.habitLogs.filter(l=>l.id!==localId&&!(String(l.habit_id)===String(habitId)&&l.date===ds));save();renderHabitsHighlight();if(typeof renderHabitsPage==='function'&&activePg==='habits')renderHabitsPage();sbReqSilent('DELETE','habit_logs',null,`?habit_id=eq.${habitId}&date=eq.${ds}`);},'Checked habit');
  }else if(!done&&wasDone){
    const removed=st.habitLogs.filter(l=>String(l.habit_id)===String(habitId)&&l.date===ds);
    st.habitLogs=st.habitLogs.filter(l=>!(String(l.habit_id)===String(habitId)&&l.date===ds));
    save();renderHabitsHighlight();if(typeof renderHabitsPage==='function'&&activePg==='habits')renderHabitsPage();
    await sbReqSilent('DELETE','habit_logs',null,`?habit_id=eq.${habitId}&date=eq.${ds}`);
    pushUndo(()=>{st.habitLogs.push(...removed);save();renderHabitsHighlight();if(typeof renderHabitsPage==='function'&&activePg==='habits')renderHabitsPage();removed.forEach(r=>sbReqSilent('POST','habit_logs',{habit_id:r.habit_id,date:r.date},''));},'Unchecked habit');
  }
}
...
function tRowVidVirt(t,arr){
  ...
  const _steps3=typeof VID_STEPS_CORE!=='undefined'?VID_STEPS_CORE:(typeof VID_STEPS!=='undefined'?VID_STEPS:[]);
  let _pct3='';
  if(_v3){const _app3=_steps3.filter(ss=>_v3[ss]!=='na');const _dn3=_app3.filter(ss=>_v3[ss]==='done').length;_pct3=_app3.length?Math.round(_dn3/_app3.length*100):0;}
  // Timeblock collapsed (list-only) = no expanded timeblock panel to see per-stage progress on, so show
  // the same 5 step dots the videos popup uses (still click-to-toggle/drag-to-timeblock) instead of just
  // a bare percentage. Expanded view keeps the percentage as before — the timeblock itself shows the
  // detail there. Only for whole-video rows (`_type==='vid'`), not per-stage vidstep rows (tRowVidStepVirt).
  const _showStages=_todTbCollapsed&&!ov&&_v3&&typeof _vidOvStepDots==='function';
  return`<div class="ti ${t.done?'done':''} ${ov?'ov-row':''}" style="background:${_vs.bg}" id="ti-${t.id}" draggable="true"
    ...
    ${_showStages?`<div style="display:flex;gap:0;flex-shrink:0;align-items:center;margin-left:auto" onclick="event.stopPropagation()">${_vidOvStepDots(_v3,_steps3)}</div>`:(!ov?`<span style="font-size:9px;opacity:.5;margin-left:auto;flex-shrink:0">${_pct3}%</span>`:'')}
  ...
  dates.forEach((date,di)=>{
    ...
    col.addEventListener('drop',async e=>{
      ...
      if(dragId.startsWith('vid::')){
        ...
        // Merge the dragged item's own undo (pushed inside _vidAssignToDay) with the other selected
        // items' undo into ONE pushUndo, so a single Undo reverts the whole group — previously these
        // were two separate undo-stack entries, so one Undo press only reverted the "other" items and
        // left the actually-dragged item still moved (2026-08-25 fix, reported as "only undoing one at
        // a time"). Safe pop: only takes the entry if _vidAssignToDay actually pushed one (it can
        // no-op via an early-return guard, e.g. "already on this day").
        const _lenBefore=undoStack.length;
        _vidAssignToDay(vidId,ds);
        const _selfUndo=(undoStack.length>_lenBefore)?undoStack.pop():null;
        if(_isMultiVid){
          const _vidOtherUndos=[];
          _moveOtherSelected(ds,_vidSid,_vidOtherUndos);
          save();renderAll();if(document.getElementById('tbGrid'))renderDayTB();
          if(_vidOtherUndos.length||_selfUndo)pushUndo(()=>{_vidOtherUndos.forEach(fn=>fn());if(_selfUndo)_selfUndo.fn();save();renderAll();if(document.getElementById('tbGrid'))renderDayTB();},'Moved tasks to '+ds);
        }else if(_selfUndo){
          undoStack.push(_selfUndo);
        }
        dragId=null;return;
      }
      if(dragId.startsWith('vidstep::')){
        ...
        if(_vsIsMulti&&_vsSrcDay&&_vsSrcDay!==ds){
          ...
          // Merge this item's own undo with the other selected items' undo into ONE pushUndo (below)
          // instead of two separate stack entries — see the vid:: branch above for why (2026-08-25 fix).
          const _vsSelfUndo=()=>{
          ...
          save();renderAll();if(document.getElementById('tbGrid'))renderDayTB();
          const _vsOtherUndos=[];
          if(_vsIsMultiSel)_moveOtherSelected(ds,_vsSid,_vsOtherUndos);
          if(_vsOtherUndos.length){save();renderAll();if(document.getElementById('tbGrid'))renderDayTB();}
          pushUndo(()=>{
            _vsSelfUndo();_vsOtherUndos.forEach(fn=>fn());
            save();renderAll();if(document.getElementById('tbGrid'))renderDayTB();
          },'Moved step'+(_vsOtherUndos.length?'s':''));
        } else {
          const _lenBefore=undoStack.length;
          ...
          if(_vsIsMultiSel){
            ...
            save();renderAll();if(document.getElementById('tbGrid'))renderDayTB();
            if(_vsOtherUndos2.length||_selfUndo)pushUndo(()=>{_vsOtherUndos2.forEach(fn=>fn());if(_selfUndo)_selfUndo.fn();save();renderAll();if(document.getElementById('tbGrid'))renderDayTB();},'Moved tasks to '+ds);
          }else if(_selfUndo){
            undoStack.push(_selfUndo);
          }
        }
    ...
    wrCell.addEventListener('drop',e=>{
      ...
      showWrScopePicker(e,
        ...
        ()=>{// Shift anchor — affects all future occurrences
          ...
          const _addedSnap=_wrSnapshotSchedule(rule);
...
function _wrClearPastOrphanPins(rule,isWrRule,lookback=6,skipWk=null){
  // "All future"/"this time only" must never rewrite past weeks — no freezing, no phantom-cleanup,
  // for any cadence. Past stays exactly as it was before the shift, always. Callers keep passing
  // (rule,isWrRule,lookback,skipWk) and treating the return as an undo list; always empty now.
  return [];
}
// Snapshot a WR rule's pre-shift starting_date before an "all future" anchor change, keyed to the
// week it stops applying. isWRRuleDueThisWeek (core.js) consults this so past weeks keep computing
// due-ness against the schedule that was actually in effect then — a shift only ever affects the
// current week onward. Returns true if a new segment was added (undo pops it back off).
function _wrSnapshotSchedule(rule){
  if(!rule._dateOverrides)rule._dateOverrides={};
  const wk=getWkKey(0);
...
function _wrCtxShiftScheduleOne(delta){
  ...
  const _addedSnap=!isRec?_wrSnapshotSchedule(rule):false;
...
function _vidStepTasksForDayWithOverdue(todayDs){
  ...
  (st.blocks||[]).filter(bl=>bl._vidStepVid&&bl._vidStepName&&bl.ds<=todayDs&&_isMultiStep(bl._vidStepName)).forEach(bl=>{
    const dayKey=bl._vidStepVid+'::'+bl._vidStepName+'::'+bl.ds;
...
function _vidStepTasksForDay(ds){
  ...
  (st.blocks||[]).filter(bl=>bl._vidStepVid&&bl._vidStepName&&bl.ds===ds).forEach(bl=>{
    ...
    tasks.push({id:'vidstep-'+key.replace('::','-')+'-'+ds,name:label+': '+(v.topic||v.title),category:'Videos',due_date:ds,done:isDone,_vidId:bl._vidStepVid,_vidStep:step,_virtual:true,_type:'vidstep',_vidStepDay:ds});
  });
  return tasks;
}
...
function _vidOvCloseTitleMode(){
  ...
  if(panel){const card=panel.parentElement;panel.style.right='0';panel.style.left='0';panel.style.width='';if(card){card.style.overflow='hidden';card.style.zIndex='';}}
  _renderVidOvMenu();
}
function _vidOvToggleTitleMode(){
  ...
  if(panel){
    ...
    // Position by explicit left/width in page coordinates (content-rect-based, see
    // _vidOvContentRect) instead of a `right` offset relative to the card's own edge — the card
    // sits inside .overview-cols, which centers itself with empty margin on each side once the
    // viewport is wider than its 1600px cap, so "card's left" isn't the same as the actual content
    // area's left; a right-offset computed against cardWidth alone silently inherited that gap and
    // undershot the true content width (2026-08-25 fix).
    const _cr=_vidOvContentRect();
    if(_cr){
      const cardRect=card.getBoundingClientRect();
      panel.style.left=(_cr.left-cardRect.left)+'px';
      panel.style.width=_cr.width+'px';
...
function _vidOvHighlight(){const rows=_vidOvGetRows();rows.forEach(r=>{r.classList.toggle('vid-sel',_vidOvSelSet.has(r.dataset.vidrow));});if(typeof _vidCalHighlightChip==='function')_vidCalHighlightChip(_vidOvSelVid);_vidCrossSyncSel();}
// Echoes the Overview panel's video selection into every other view. The Today list, week/day
// timeblock chips, and calendar grid already highlight matching 'vid-ov-<id>'/'vidstep-<id>-<step>'
// entries for free via the app-wide selectedTasks/applySelHighlight system, so we just need to keep
// that Set in sync with _vidOvSelSet. The full Videos page and the All Videos panel aren't wired
...
function _renderVidOvMenu(){
  ...
  if(_focusActive){
    ...
    // The video's own day-map slot / post_date only keep it in focus while NOT done — same "done
    // instance drops focus" rule _vidHasStepInWk already applies per-step. Without this, a video
    // whose calendar task or post_date fell earlier this week but is already published (or, for a
    // step-only checkbox video, past+complete) kept the yellow highlight all week regardless
    // (2026-08-27 fix — these two checks had no completion gate at all, unlike the step check).
    vids.forEach(vid=>{
      const sid=String(vid.id);
      const dayDs=_map[sid];
      const _done=vid.status==='published';
      if((!_done&&(_inWk(dayDs)||_inWk(vid.post_date)))||_vidHasStepInWk(vid))_focusSet.add(sid);
      (st.videos||[]).filter(c=>!c.is_deleted&&String(c.big_video_id)===sid).forEach(c=>{
        const csid=String(c.id);
        const cd=_map[csid];
        const _cDone=c.status==='published';
        if((!_cDone&&(_inWk(cd)||_inWk(c.post_date)))||_vidHasStepInWk(c)){_focusSet.add(sid);_focusSet.add(csid);}
      });
    });
  }
  ...
  // Add button lives far right (matching the In Progress/Ideas toolbox headers), same position:absolute
  // technique as the title-mode close button below — an in-flow flex item here would eat into the row's
...
// Embedded beside the video list inside #vidOvPanel itself (same extend-the-real-panel
// mechanism as Edit-Title mode: panel.style.right goes negative to grow the panel past the
// card), rather than a separate floating panel over the page (2026-08-25).
let _vidCalOpen=false,_vidCalMonth=null,_vidCalYear=null;
const _VID_CAL_W=440;
function _vidOvToggleCal(){
  ...
  if(panel){
    ...
    // Fill the full page content width, same as Title mode, instead of a fixed sidebar width —
  ...
  _renderVidOvMenu();
  // Close only on clicks truly outside the panel/card, Escape, or M key
  setTimeout(()=>{
    const _calClose=e=>{
      if(!_vidCalOpen)return;
      if(!document.body.contains(e.target))return;
      // Don't close if clicking inside the video panel (calendar is now part of it), card, overlays, or context menus
...
function _vidCalChipColor(v,ds,today){
  ...
  const dk=_dk();
  if(isDone||isPast){
    return isBig?{bg:dk?'rgba(16,185,129,.20)':'rgba(16,185,129,.15)',fg:dk?'#34d399':'#059669'}:{bg:dk?'rgba(101,163,13,.18)':'rgba(101,163,13,.10)',fg:dk?'#a3e635':'#65a30d'};
  }
  return isBig?{bg:dk?'rgba(14,100,210,.22)':'rgba(14,100,210,.14)',fg:dk?'#60a5fa':'#1d4ed8'}:{bg:dk?'rgba(14,165,233,.20)':'rgba(14,165,233,.10)',fg:dk?'#38bdf8':'#0ea5e9'};
}
function _vidCalRenderMonth(y,m,vidsByDate,today,search){
  ...
  let html=`<div style="display:flex;align-items:stretch;margin-bottom:4px;background:${dk?'rgba(255,255,255,.03)':'rgba(255,255,255,.5)'};backdrop-filter:blur(8px);border:1px solid ${dk?'rgba(255,255,255,.08)':'rgba(210,205,228,.15)'};border-radius:8px;overflow:hidden${isNewYear?';margin-top:8px':''}">`;
  // Left label: month on top, divider, year below
  html+=`<div style="width:38px;flex-shrink:0;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:4px 2px;background:${isCurMonth?(dk?'rgba(249,115,22,.10)':'rgba(249,115,22,.06)'):(dk?'rgba(255,255,255,.03)':'rgba(120,113,145,.03)')};border-right:1px solid ${dk?'rgba(255,255,255,.06)':'rgba(210,205,228,.1)'}">`;
...
function _vidCalHighlightChip(vidId){
  const cal=document.getElementById('vidOvCalEmbed');
  if(!cal)return;
...
// Content-area rect for title-mode/calendar panel extension: #main has no max-width
...
function _vidOvRenderCal(){
  // Renders into the #vidOvCalEmbed column that _renderVidOvMenu injects into #vidOvPanel
  // itself when _vidCalOpen — no longer a separate floating panel over the page (2026-08-25).
  const panel=document.getElementById('vidOvCalEmbed');
  if(!panel)return;
  const today=d2s(new Date());
...
function _vidOvAllProgRow(v){
  ...
  // Big row now matches Up Next's own treatment (and Ideas' rows): plain var(--muted)/500 title, no
  // bold var(--text) and no "· N" child-count badge next to it — the per-child numbering below already
  // shows the count implicitly, same as the Up Next list (2026-09-14, per explicit request).
  let html=`<div data-vidrow="${sid}" data-alldrag="${sid}" draggable="true" ondragstart="dragId='vid::${sid}';event.dataTransfer.effectAllowed='move'" ondragover="_vidOvNestDragOver(event)" ondragleave="_vidOvNestDragLeave(event)" ondrop="_vidOvNestDrop(event,'${sid}')" onclick="_voaRowClick(event,'${sid}')" ondblclick="if(typeof openVidEdit==='function')openVidEdit('${sid}')" oncontextmenu="if(typeof showVidCtx==='function')showVidCtx(event,'${sid}')" class="${_sel?'vid-sel':''}" style="padding:5px 6px;border-radius:6px;font-size:12px;font-weight:500;color:var(--muted);cursor:grab;display:flex;align-items:center;gap:5px" onmouseenter="if(!this.classList.contains('vid-sel'))this.style.background='${_hovBg}'" onmouseleave="if(!this.classList.contains('vid-sel'))this.style.background=''">${_addBtn}<span style="flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;line-height:12px">${escHtml(v.topic||v.title)}</span><div style="display:flex;flex-shrink:0;align-items:center">${_vidOvStepDots(v,['step_build'])}</div><div class="vid-ov-pctx" style="width:14px;flex-shrink:0;position:relative;margin-left:2px;display:flex;align-items:center;justify-content:flex-end;line-height:12px">${_xBtn}</div></div>`;
  // Show children (small videos) — numbered "1.", "2." like Up Next's own child rows, instead of just
  // a bare └ with no index (2026-09-14, per explicit request to make this list read more like Up Next).
  if(v.video_type==='B'){
...
function _vidOvRenderAll(){
  ...
  // strip across the list + both toolbox columns (2026-09-14). border-bottom:none overrides
  // .tod-tb-header's default rule under the text — Up Next's own header has no such rule, so these two
  ...
  if(bigIdeas.length||littleIdeas.length){
    ...
    // are plain block content with no scroll of their own; the squeeze doesn't clip them, it just lets
...
function _vidOvRenderAnalyticsPanel(){
  ...
  const _anCloseBtn=`<button onclick="_vidOvCloseAnalytics()" title="Close" style="position:absolute;top:10px;right:10px;z-index:5;background:none;border:none;cursor:pointer;color:var(--muted);padding:3px;border-radius:4px;display:flex;align-items:center;justify-content:center"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.25" stroke-linecap="square" stroke-linejoin="round"><line x1="19" y1="5" x2="5" y2="19"/><line x1="5" y1="5" x2="19" y2="19"/></svg></button>`;
  if(typeof _vidRenderAnalytics==='function'){
    const content=_vidRenderAnalytics();
    panel.innerHTML=`<div style="position:relative;width:100%;height:100%;overflow-y:auto">${_anCloseBtn}${content}</div>`;
  }else{
    panel.innerHTML=`<div style="position:relative;width:100%;height:100%">${_anCloseBtn}<div style="padding:40px;text-align:center;color:var(--muted);font-size:12px">Analytics not available</div></div>`;
  }
}
...
function tRow(t,o={}){
  ...
  const _noteAttr=_hasNotes?` data-note="${escHtml(t.notes.trim())}"`:'';
  return`<div class="ti ${t.done?'done':''} ${ov?'ov-row':''} ${imp&&!ov?'imp-row':''}" style="${!ov&&!imp&&!o.noColor?`background:${s.bg}`:''}" id="ti-${t.id}"${_noteAttr} ${o.drag?`draggable="true" ondragstart="dStart(event,'${t.id}')" ondragend="dEnd(event)"`:''} onclick="selTask(event,'${t.id}')" ondblclick="${_dblHandler}" oncontextmenu="showCtx(event,'${t.id}')">
    ${o.catDot?`<span class="ti-band${_hasNotes?' ti-band-note':''}" style="background:${s.bg};border:1px solid ${s.d}${_hasNotes?`;box-shadow:0 0 4px 1px ${s.d}`:''}"></span>`:''}
