...
function renderPupTable(){
  ...
  groups.forEach(g=>{
    ...
    const pupCells=pups.map(p=>{
      ...
      if(!s)return`<td style="padding:2px 5px" title="Not assigned to ${p} — double-click row to edit"><div style="${pillBase};grid-template-columns:1fr;justify-items:center;opacity:.3"><span style="font-size:10px;color:var(--muted)">—</span></div></td>`;
      ...
      if(isNotStarted){
        return`<td onclick="event.stopPropagation();pupStageClick('${sid}')" style="padding:2px 5px;cursor:pointer" title="Not started — click to start"><div style="${pillBase};grid-template-columns:1fr;justify-items:center;background:transparent;border-style:dashed;border-color:${col}45;box-shadow:none;opacity:.55"><span style="font-size:9px;color:var(--muted);letter-spacing:.4px">start</span></div></td>`;
      }
      let stageWidget;
    ...
    const tipAttr=hasTip?` onmouseenter="showPupRichTip(event,${tipRows.replace(/"/g,'&quot;')})" onmouseleave="hidePupTip()"`:'';
