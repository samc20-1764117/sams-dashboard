...
if(!window._BUILD&&!sessionStorage._bust){sessionStorage._bust='1';location.href='/?_='+Date.now();}
...
function _syncPX(){const sc=document.getElementById('tbScroll');if(sc){const visH=sc.clientHeight;if(visH<=0)return;const hourH=visH/14;document.documentElement.style.setProperty('--hour-h',hourH+'px');PX=hourH/60;}}
...
let st={tasks:[],recurring:[],shopping:[],blocks:[],travel:[],birthdays:[],ideas:[],pup_skills:[],pupSessions:[],pupWeeklyFocus:[],recipes:[],videos:[],autoTimeblocks:[],autoTBOverrides:[],wrRules:[],wrOverrides:[],groceryStaples:[],groceryList:[],mealPlan:[],packTemplates:[],packItems:[],finance:[],finSubs:[],finPoints:[],easyMeals:[],_grocStapleSkips:{},_holidayDates:{}};
...
async function syncAll(silent=false){
  ...
  try{
    ...
    const[tasks,shop,trav,bdays,pupSkills,pupSessionsDb,pupWkFocusDb,recipes,videosDb,gStaples,gList,mealPlanDb,packTplDb,packItemDb,finDb,finSubDb,finPointsDb,ideasDb,clientKv]=await Promise.all([
    ...
    if(finPointsDb){
      const dbIds=new Set(finPointsDb.map(r=>String(r.id)));
      const localOnly=st.finPoints.filter(r=>String(r.id).startsWith('l-')&&!dbIds.has(String(r.id)));
      st.finPoints=[...finPointsDb,...localOnly];
    }
    ...
    if(wrRules){
      ...
      // is intentionally kept under its ORIGINAL week key with an out-of-week (future) date, so it stays a
      // SEPARATE instance from that later week's own occurrence (e.g. last week's HEB landed on this Monday
      // while this week's HEB stays on its Sunday). Those forward pins MUST be preserved as-is.
      // WR rules → drop out-of-week pins (the wr_recurring_overrides move/schedule shows the instance).
...
document.addEventListener('keydown',e=>{
  ...
  // C = Cinema (bare key, same pattern as b/p/f/h/l page toggles)
  if(e.key==='c'&&!e.metaKey&&!e.ctrlKey&&!document.querySelector('input:focus,textarea:focus,select:focus,[contenteditable="true"]:focus')&&!document.querySelector('.overlay.open')){
    e.preventDefault();if(activePg==='cinema')showPage('overview');else showPage('cinema');
  }
