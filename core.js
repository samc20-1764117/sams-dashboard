...
let st={tasks:[],recurring:[],shopping:[],blocks:[],travel:[],birthdays:[],pup_skills:[],pupSessions:[],pupWeeklyFocus:[],recipes:[],videos:[],autoTimeblocks:[],autoTBOverrides:[],wrRules:[],wrOverrides:[],groceryStaples:[],groceryList:[],mealPlan:[],packTemplates:[],packItems:[],finAccounts:[],finSubs:[],finVTI:[],_grocStapleSkips:{}};
...
async function syncAll(silent=false){
  ...
  try{
    const[tasks,shop,trav,bdays,pupSkills,pupSessionsDb,pupWkFocusDb,recipes,videosDb,gStaples,gList,mealPlanDb,packTplDb,packItemDb,finAccDb,finSubDb,finVTIDb]=await Promise.all([
      ...
      sbReqSilent('GET','finance_accounts',null,'?order=sort_order.asc.nullslast,name.asc&select=*'),
      sbReqSilent('GET','finance_subs',null,'?order=sort_order.asc.nullslast,name.asc&select=*'),
      sbReqSilent('GET','finance_vti',null,'?order=date.asc&select=*')
    ]);
    ...
    if(finAccDb)st.finAccounts=finAccDb;
    if(finSubDb)st.finSubs=finSubDb;
    if(finVTIDb)st.finVTI=finVTIDb;
