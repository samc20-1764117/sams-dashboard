...
function mShowTab(tab) {
  ...
  document.getElementById('mApp').style.paddingBottom = 'calc(78px + env(safe-area-inset-bottom))';
...
function mSyncBarClearance(barId, listId) {
  requestAnimationFrame(() => {
    ...
    // Bar sits at bottom:calc(76px + safe-area) — its own height stacks on top of that,
    // so the list needs both plus a small buffer to fully clear it. 76 must match
    // #mAddBar/#mShopAddBar's own bottom offset in mobile.css.
    if (h > 0) list.style.paddingBottom = `calc(${h + 76 + 24}px + env(safe-area-inset-bottom))`;
  });
}
