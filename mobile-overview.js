...
function mShowTab(tab) {
  ...
  document.getElementById('mApp').style.paddingBottom = 'calc(72px + env(safe-area-inset-bottom))';
...
function mSyncBarClearance(barId, listId) {
  requestAnimationFrame(() => {
    ...
    // Bar sits at bottom:calc(70px + safe-area) — its own height stacks on top of that,
    // so the list needs both plus a small buffer to fully clear it. 70 must match
    // #mAddBar/#mShopAddBar's own bottom offset in mobile.css.
    if (h > 0) list.style.paddingBottom = `calc(${h + 70 + 24}px + env(safe-area-inset-bottom))`;
  });
}
