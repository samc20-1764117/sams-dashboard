...
const VID_STEP_LABELS={step_build:'Build',step_vo:'Vo',step_film:'Film',step_cut:'Cut',step_thumbnail:'Th',step_description:'Des',step_tableau_public:'Tab',step_upload_tableau:'Update'};
...
function _vidDashList(vids,simple){
  ...
  sorted.forEach(v=>{
    ...
    if(v.video_type==='B'){
      ...
      children.forEach((l,ci)=>{seen.add(String(l.id));html+=_vidDashRow(l,true,simple);if(!simple&&ci<children.length-1){const oA=l.vid_order??ci;const oB=children[ci+1].vid_order??(ci+1);html+=`<div class="vid-insert-zone"><button class="vid-insert-btn" onclick="event.stopPropagation();openVidModalBetween('${String(v.id)}',${oA},${oB})" title="Insert video here">+</button></div>`;}});
...
function _vidBuildRows(vids){
  ...
  bSorted.forEach(b=>{
    ...
    children.forEach((l,ci)=>{seen.add(String(l.id));html+=_vidRow(l,true);if(ci<children.length-1){const oA=l.vid_order??ci;const oB=children[ci+1].vid_order??(ci+1);html+=`<tr class="vid-insert-zone"><td colspan="99" style="padding:0;border:none"><button class="vid-insert-btn" onclick="event.stopPropagation();openVidModalBetween('${String(b.id)}',${oA},${oB})" title="Insert video here">+</button></td></tr>`;}});
  });
