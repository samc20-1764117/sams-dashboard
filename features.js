...
function renderPackingModal(travelId){
  ...
  _cols.forEach(colCats=>{
    ...
    colCats.forEach(cat=>{
      ...
      html+=`<div style="margin-bottom:14px;position:relative;padding:4px 6px;border-radius:6px;transition:background .15s" onmouseenter="this.style.background='rgba(0,0,0,.02)';var c=this.querySelector('.pack-cat-chk');c.style.width='20px';c.style.opacity='1';this.querySelector('.pack-cat-del').style.opacity='1'" onmouseleave="this.style.background='';var c=this.querySelector('.pack-cat-chk');c.style.width='0px';c.style.opacity='0';this.querySelector('.pack-cat-del').style.opacity='0'"><div style="font-size:11px;font-weight:600;color:var(--muted);margin-bottom:3px;display:flex;align-items:center;gap:0"><label class="chk-wrap pack-cat-chk" style="margin:0;opacity:0;width:0;overflow:hidden;transition:width .2s ease,opacity .2s ease;flex-shrink:0;padding-right:0" onclick="event.stopPropagation()"><input type="checkbox" class="chk" ${allChecked?'checked':''} onchange="_packCheckAllCat('${travelId}','${cat}',this.checked)"></label><span style="transition:transform .2s ease">${escHtml(cat)}</span><button class="pack-cat-del" onclick="event.stopPropagation();_delPackCat('${travelId}','${cat}')" style="opacity:0;margin-left:auto;background:none;border:none;cursor:pointer;font-size:11px;color:var(--muted);padding:0 2px;transition:opacity .15s" title="Remove all ${cat} items from this trip">&#10005;</button></div>`;
