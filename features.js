...
function renderShopFull(){save();
  ...
  if(mode==='manual'){
    ...
    sorted.forEach(s=>{
      ...
      el.innerHTML=`<input type="checkbox" class="chk"${s.done?' checked':''}><span class="tn">${escHtml(s.name)}</span><span class="cpill" style="background:none;color:#94a3b8;border:none;box-shadow:none;flex-shrink:0;margin-left:auto;margin-right:2px">${escHtml(s.store||'Other')}</span><button class="delbtn">✕</button>`;
...
function showPage(id){
  ...
  if(id==='weekly'){renderWeeklyPage();}if(id==='travel')renderTravelPage();if(id==='birthdays')renderBdayPage();if(id==='pups')renderPupsPage();if(id==='recipes')renderRecipesPage();if(id==='overview'){renderShopOv();renderRecOv();}
