...
function rtDblEdit(td, rid, field){
  ...
  function commit(){
    ...
    if(newVal!==String(val)){
      ...
      sbReq('PATCH','recurring_tasks',{[field]:r[field]},recQs(rid));
...
function rtTogglePup(rid){
  ...
  sbReq('PATCH','recurring_tasks',{pup_related:r.pup_related},recQs(rid));
...
async function duplicateRecDirect(rid){
  ...
  pushUndo(()=>{
    ...
    save();renderRecOv();renderWeeklyPage();renderToday();renderWkSummary();renderWkCal();
    if(sv&&sv[0])sbReq('DELETE','recurring_tasks',null,recQs(sv[0].id));
...
function togRec(id,done,wkKey){
  ...
  save();renderRecOv();renderWeeklyPage();renderToday();renderWkSummary();renderWkCal();
  if(document.getElementById('tbGrid'))renderDayTB();
  sbReq('PATCH','recurring_tasks',{done_by_week:r._doneByWk},recQs(id));
  pushUndo(()=>{
    r._done=prev;
    ...
    save();renderRecOv();renderWeeklyPage();renderToday();renderWkSummary();renderWkCal();
    if(document.getElementById('tbGrid'))renderDayTB();
    sbReq('PATCH','recurring_tasks',{done_by_week:prevByWk},recQs(id));
...
async function delRec(id){
  ...
  await sbReq('DELETE','recurring_tasks',null,recQs(sid));
...
async function saveRecModal(){
  ...
  pushUndo(()=>{const rid=recServerId||localId;st.recurring=st.recurring.filter(x=>String(x.id)!==String(rid));save();renderRecOv();renderWeeklyPage();renderWkSummary();renderWkCal();if(recServerId)sbReq('DELETE','recurring_tasks',null,recQs(recServerId));},'Added recurring task');
...
function unscheduleWRec(rid,wkKey){
  ...
  pushUndo(()=>{
    ...
    linkedBlocks.forEach(b=>{if(st.blocks)st.blocks.push(b);sbSaveBlock(b);});
    save();renderAll();
    sbReq('PATCH','recurring_tasks',{date_overrides:r._dateOverrides},recQs(rid));
  },'Removed from view');
  sbReq('PATCH','recurring_tasks',{date_overrides:r._dateOverrides},recQs(rid));
}
...
function openMModal(){
  ...
  if(!modal._wheelBound){
    ...
    let _moWheelAcc=0,_moWheelLast=0;
    modal.addEventListener('wheel',e=>{
      // Only act if not scrolling inside mUAList
      if(e.target.closest('.mual')||e.target.closest('.mua-side'))return;
      e.preventDefault();
      const now=Date.now();if(now-_moWheelLast>600)_moWheelAcc=0;
      _moWheelAcc+=e.deltaY;_moWheelLast=now;
      if(Math.abs(_moWheelAcc)<200)return;
      shiftMo(_moWheelAcc>0?1:-1);_moWheelAcc=0;
    },{passive:false});
  }
}
...
function addMoTravelBanners(cells,yr,mo){
  ...
  cells.style.position=''; // reset so cells don't become a containing block
  let layer=document.getElementById('moBanners');
  if(layer)layer.remove();
  layer=document.createElement('div');layer.id='moBanners';
  layer.style.cssText='position:absolute;top:0;left:0;right:0;height:0;overflow:visible;pointer-events:none;z-index:3';
  mgrid.appendChild(layer);
  if(!trips.length)return;
  trips.sort((a,b)=>(a.start_date||'').localeCompare(b.start_date||''));
  setTimeout(()=>{
    const cellEls=[...cells.querySelectorAll('.mcell')];if(!cellEls.length)return;
    // Lane tracking per calendar row (keyed by offsetTop)
    // Each entry: array of {si,ei,lane} where si/ei are column indices 0-6
    const rowLanes=new Map();
    function getMoLane(rowTop,si,ei){
      if(!rowLanes.has(rowTop))rowLanes.set(rowTop,[]);
      const occ=rowLanes.get(rowTop);
      for(let lane=0;;lane++){
        const conflict=occ.some(r=>r.lane===lane&&!(ei<r.si||si>r.ei));
        if(!conflict){occ.push({si,ei,lane});return lane;}
      }
    }
    // Compute column index (0-6) for each cell within its row
    const cellColIdx=new Map();
    const rowGroups=new Map();
    cellEls.forEach(c=>{
      const rt=c.offsetTop;
      if(!rowGroups.has(rt))rowGroups.set(rt,[]);
      const ci=rowGroups.get(rt).length;
    ...
    trips.forEach(tv=>{
      ...
      for(let i=1;i<rangeCells.length;i++){
        if(rangeCells[i].offsetTop!==rangeCells[i-1].offsetTop){rows.push(cur);cur=[rangeCells[i]];}else cur.push(rangeCells[i]);
      }
      rows.push(cur);
      rows.forEach((rowCells,ri)=>{
        const fc=rowCells[0],lc=rowCells[rowCells.length-1];
...
async function saveTravelModal(){
  ...
  if(travelEditId){
    ...
    pushUndo(()=>{Object.assign(tv,prev);renderAll();renderTravelPage();const _tid2=String(travelEditId);pendingTravelIds.add(_tid2);sbReq('PATCH','travel',prev,`?id=eq.${travelEditId}`).then(()=>pendingTravelIds.delete(_tid2));},'Edited trip');
...
// ── Sidebar ────────────────────────────────────────────────────────────────────
function closeSB(){sbOpen=false;document.getElementById('sidebar').classList.add('closed');document.getElementById('main').style.left='0';document.getElementById('menuOpen').classList.add('visible');save();}
function openSB(){sbOpen=true;document.getElementById('sidebar').classList.remove('closed');document.getElementById('main').style.left='186px';document.getElementById('menuOpen').classList.remove('visible');save();}
...
function init(){
  ...
  if(!sbOpen){document.getElementById('sidebar').classList.add('closed');document.getElementById('main').style.left='0';document.getElementById('menuOpen').classList.add('visible');}
...
document.addEventListener('keydown',async e=>{
  ...
  if((e.key==='Delete'||e.key==='Backspace')&&selectedTasks.size>0){
    ...
    await Promise.all([
      ...
      ...recIds.map(rid=>sbReq('DELETE','recurring_tasks',null,recQs(rid))),
...
function saveRecEdit(){
  ...
  sbReq('PATCH','recurring_tasks',patch,recQs(rid));
...
async function rolloverOverdue(){
  ...
  await Promise.all([
    ...
    ...recsToPatch.map(rid=>{const r=st.recurring.find(x=>String(x.id)===String(rid));return r?sbReq('PATCH','recurring_tasks',{date_overrides:r._dateOverrides},recQs(r.id)):Promise.resolve();})
  ]);
}
