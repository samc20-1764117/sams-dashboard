...
function renderRtGroup(containerId, tasks, cadence){
  ...
  tasks.forEach(r=>{
    ...
    tbody+=`<tr class="rt-row" id="ti-rt-${rid}" onclick="selTask(event,'${virtId}')" ondblclick="if(!event.target.closest('.delbtn')&&!event.target.closest('.btn-xs')){event.stopPropagation();openRecEditModal('${rid}');}" oncontextmenu="showRecCtx(event,'${rid}',getWkKey(wkOff))">
      ${tds}
...
function showMcellMorePop(e,tasks,ds){
  ...
  tasks.forEach(t=>{
    ...
    chip.addEventListener('contextmenu',ev=>{if(isTravel){ev.preventDefault();ev.stopPropagation();closeMorePop();openTravelModal(t._srcId);}else if(t._type==='shop')showCtx(ev,null,false,null,t._shopId);else if(t._isWrRule)showWrRuleCtx(ev,String(t._ruleId),t._wkKey||getWkKey(wkOff));else if(t._isWrec||t._recId)showRecCtx(ev,String(t._recId),t._wkKey||getWkKey(wkOff));else if(!t._virtual)showCtx(ev,t.id);});
