...
function openQA(ctx,btn,ds='',kcat=''){
  ...
  if(ctx==='pup'){
    ...
    <div class="qa-field"><label>Link <span style="opacity:.45;font-weight:400">(optional)</span></label><input id="qaShopLink" type="text" placeholder="https://…" style="${_sStyle}"></div>`;
  } else if(ctx==='rec'){
...
let _cinemaWatchedSort=null; // null | 'asc' | 'desc'
let _cinemaLastUpNextShows=[];
let _cinemaLastUpNextMovies=[];
function _cinemaToggleWatchedSort(){
...
function _cinemaGenresIn(items){
  const set=new Set();
  items.forEach(i=>(i.genre||'').split(',').map(s=>s.trim()).filter(Boolean).forEach(g=>set.add(g)));
  return[...set].sort();
}
function renderCinemaPage(){
  const upShowsEl=document.getElementById('cinemaUpNextShowsContent');
  if(!upShowsEl)return;
  const upMoviesEl=document.getElementById('cinemaUpNextMoviesContent');
  const watchShowsEl=document.getElementById('cinemaWatchedShowsContent');
  const watchMoviesEl=document.getElementById('cinemaWatchedMoviesContent');
  ...
  const wGenreFilterEl=document.getElementById('cinemaWatchedGenreFilter');

  const upNextAll=st.cinemaItems.filter(i=>i.status==='up_next').sort((a,b)=>(a.sort_order??9999)-(b.sort_order??9999));
  const genres=_cinemaGenresIn(upNextAll);
  ...
  const upNextShows=upNext.filter(i=>i.type==='show');
  const upNextMovies=upNext.filter(i=>i.type==='movie');
  _cinemaLastUpNextShows=upNextShows;
  _cinemaLastUpNextMovies=upNextMovies;

  const watchedAll=st.cinemaItems.filter(i=>i.status==='watched').sort((a,b)=>new Date(b.updated_at||b.created_at)-new Date(a.updated_at||a.created_at));
  const wGenres=_cinemaGenresIn(watchedAll);
  if(!wGenres.includes(_cinemaWatchedGenreFilterVal))_cinemaWatchedGenreFilterVal='';
  if(wGenreFilterEl)wGenreFilterEl.innerHTML='<option value="">All genres</option>'+wGenres.map(g=>`<option value="${escHtml(g)}" ${g===_cinemaWatchedGenreFilterVal?'selected':''}>${escHtml(g)}</option>`).join('');
  let watched=watchedAll;
  if(_cinemaWatchedGenreFilterVal)watched=watched.filter(i=>(i.genre||'').split(',').map(s=>s.trim()).includes(_cinemaWatchedGenreFilterVal));
  const watchedShows=watched.filter(i=>i.type==='show');
  const watchedMovies=watched.filter(i=>i.type==='movie');
  if(_cinemaWatchedSort){
    const dir=_cinemaWatchedSort==='asc'?1:-1;
    const byRating=(a,b)=>((a.rating??-1)-(b.rating??-1))*dir;
    watchedShows.sort(byRating);watchedMovies.sort(byRating);
  }
...
function _cinemaNewSeasonBadge(item){
  return item.new_season?`<span title="New season out" style="flex-shrink:0;font-size:8px;font-weight:700;text-transform:uppercase;letter-spacing:.02em;padding:1px 4px;border-radius:4px;background:rgba(245,158,11,.15);color:#b45309">New</span>`:'';
}
...
function _cinemaRowShell(item,inner,draggable,extraAttrs,compact){
...
function _cinemaWatchBtn(item,watched){
  const action=watched?'moveCinemaToUpNext':'moveCinemaToWatched';
  const title=watched?'Move back to Watchlist':'I watched this';
...
function _cinemaUpNextRow(item){
  return _cinemaRowShell(item,`
    ${_cinemaWatchBtn(item,false)}
    ${_cinemaTitleLine(item)}
    ${_cinemaNewSeasonBadge(item)}
...
function _cinemaWatchedRow(item){
  return _cinemaRowShell(item,`
    ${_cinemaWatchBtn(item,true)}
    ${_cinemaTitleLine(item)}
    ${_cinemaNewSeasonBadge(item)}
    <input type="number" min="1" max="10" step="0.1" value="${item.rating??''}" placeholder="–" class="cinema-rating-input" onclick="event.stopPropagation()" onchange="setCinemaRating('${item.id}',this.value)" title="Your rating (1-10)" style="width:30px;flex-shrink:0;text-align:right;padding:1px 2px;border-radius:6px;border:1px solid var(--border);background:transparent;color:var(--text);font-family:inherit;font-size:11px">
    <button onclick="event.stopPropagation();deleteCinemaItem('${item.id}')" class="idea-x-btn" style="background:none;border:none;cursor:pointer;font-size:10px;color:var(--muted);padding:1px 3px;flex-shrink:0;opacity:0;transition:opacity .15s">✕</button>
...
function _cinemaContainerDblClick(e,status,type){
  if(e.target.closest('.cinema-row'))return;
  openCinemaModal(null,status,type);
}
...
function openCinemaModal(editId,defaultStatus,defaultType){
  ...
  document.getElementById('cinemaMTitle').textContent=editId?'Edit':'Add';
...
async function saveNewCinemaItem(){
  ...
  const type=(document.getElementById('cinemaTypeInput')||{}).value||'show';
  ...
  const new_season=document.getElementById('cinemaNewSeasonInput').checked;
  ...
  const row={id:localId,title,type,status,rating,genre,where_to_watch,notes,new_season,sort_order,created_at:new Date().toISOString(),updated_at:new Date().toISOString()};
  st.cinemaItems.push(row);renderCinemaPage();save();
  const sv=await sbReq('POST','cinema_items',{title,type,status,rating,sort_order,genre,where_to_watch,notes,new_season});
...
async function saveCinemaEdit(id){
  ...
  Object.assign(item,{title,type,rating,genre,where_to_watch,notes,status,new_season,sort_order,updated_at:new Date().toISOString()});
  renderCinemaPage();save();
  if(!String(id).startsWith('l-'))await sbReq('PATCH','cinema_items',{title,type,rating,genre,where_to_watch,notes,status,new_season,sort_order},`?id=eq.${id}`);
}
...
async function moveCinemaSelectedToUpNext(ids){
  ...
  renderCinemaPage();save();
  const label=items.length>1?`Moved ${items.length} items to Watchlist`:'Moved "'+items[0].title+'" to Watchlist';
  pushUndo(()=>{
...
async function deleteCinemaItems(ids){
  ...
  pushUndo(async()=>{
    removed.forEach(r=>st.cinemaItems.push(r));renderCinemaPage();save();
    // Re-insert server-side too — the DELETE below already removed these rows from Supabase,
    // so without a re-POST here the undo only lives in localStorage and vanishes on next refresh/sync.
    for(const r of removed){
      if(String(r.id).startsWith('l-'))continue;
      const{id,created_at,updated_at,...body}=r;
      const sv=await sbReq('POST','cinema_items',body);
      if(sv&&sv[0]){const idx=st.cinemaItems.findIndex(x=>String(x.id)===String(id));if(idx>=0)st.cinemaItems[idx]=sv[0];renderCinemaPage();save();}
    }
  },label);
...
function _cinemaSetupReorderZone(zoneId,listId,arrGetter){
  ...
  zone.addEventListener('drop',e=>{
    ...
    renderCinemaPage();save();
    items.forEach(it=>{if(!String(it.id).startsWith('l-'))sbReqSilent('PATCH','cinema_items',{sort_order:it.sort_order},`?id=eq.${it.id}`);});
    pushUndo(()=>{
      prevOrders.forEach(({id,order})=>{const it=st.cinemaItems.find(x=>String(x.id)===String(id));if(it){it.sort_order=order;if(!String(id).startsWith('l-'))sbReqSilent('PATCH','cinema_items',{sort_order:order},`?id=eq.${id}`);}});
      save();renderCinemaPage();
    },'Reordered Watchlist');
  });
}
...
function openHabitModal(editId,defaultStage){
  ...
  const h=editId?st.habits.find(x=>String(x.id)===String(editId)):null;
  document.getElementById('habitMTitle').textContent=editId?'Edit Habit':'Add Habit';
  document.getElementById('habitSaveBtn').textContent=editId?'Save':'Add';
  document.getElementById('habitDelBtn').style.display=editId?'':'none';
...
function saveHabitModal(){
  closeMod('habitModal');
  if(_habitModalEditId)saveHabitEdit(_habitModalEditId);else saveNewHabit();
}
function _habitReadModalFields(){
...
async function saveNewHabit(){
  ...
  if(!f.name){renderHabitsPage();return;}
  const items=st.habits.filter(h=>h.stage===f.stage);
  const sort_order=items.length?Math.max(...items.map(h=>h.sort_order??0))+1:0;
  const localId='l-'+Date.now();
...
async function setHabitStage(id,stage){
  const h=st.habits.find(x=>String(x.id)===String(id));if(!h)return;
  const prevStage=h.stage,prevChangedAt=h.stage_changed_at;
  h.stage=stage;h.stage_changed_at=new Date().toISOString();
  renderHabitsPage();renderHabitsHighlight();save();
  pushUndo(()=>{h.stage=prevStage;h.stage_changed_at=prevChangedAt;renderHabitsPage();renderHabitsHighlight();save();if(!String(id).startsWith('l-'))sbReqSilent('PATCH','habits',{stage:prevStage,stage_changed_at:prevChangedAt},`?id=eq.${id}`);},'Moved "'+h.name+'" to '+stage);
  if(!String(id).startsWith('l-'))await sbReq('PATCH','habits',{stage,stage_changed_at:h.stage_changed_at},`?id=eq.${id}`);
}
async function setHabitTime(id,val){
  const h=st.habits.find(x=>String(x.id)===String(id));if(!h)return;
  h.scheduled_time=val||null;save();renderHabitsPage();
  if(!String(id).startsWith('l-'))await sbReq('PATCH','habits',{scheduled_time:h.scheduled_time},`?id=eq.${id}`);
}
async function deleteHabit(id){
  const old=st.habits.find(h=>String(h.id)===String(id));if(!old)return;
...
function deleteHabitFromModal(){
  if(!_habitModalEditId)return;
  closeMod('habitModal');
  deleteHabit(_habitModalEditId);
}
// ── Holidays ─────────────────────────────────────────────────────────────────
...
function openFinPointsDetails(){
  ...
  const rightRect=rightCol.getBoundingClientRect();
  // No width cap (unlike Purchase History's 360px) — this table has more columns
  // (Amount+Type, Expires, $ Value) and longer names ("United (VG358989)"), so it uses
  // the Recurring Expenses column's FULL width instead of leaving space unused.
  pop=document.createElement('div');pop.id='finPointsPop';pop.className='fin-details-pop';
  pop.style.cssText=`position:fixed;top:${cardRect.top}px;left:${rightRect.left}px;width:${rightRect.width}px;height:${rightRect.height}px;z-index:100`;
...
// ── Pages ──────────────────────────────────────────────────────────────────────
const PAGES=['overview','weekly','shopping','travel','birthdays','holidays','ideas','settings','pups','finance','recipes','notes','videos','cinema','packing','guide'];
// ══════════════════════════════════════════════════════════════════════════════
// ── RECIPES PAGE ──────────────────────────────────────────────────────────────
// ══════════════════════════════════════════════════════════════════════════════
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;align-content:start">
  ...
  <div style="${panel}">
    ...
    ${sRow('X / Space / Enter','Toggle checked/done for selected task(s) — Today list, weekly-cal column, or Weekly Goals, multi-select aware, single combined undo. Checking off moves selection to the next item.')}
...
function showPage(id){
  ...
  if(_navGrpTasks){
    ...
    else _navGrpTasks.classList.toggle('open',['weekly','birthdays','holidays','videos','packing','bytype'].includes(id));
  }
  ...
  if(id==='weekly'){renderWeeklyPage();}if(id==='travel')renderTravelPage();if(id==='birthdays')renderBdayPage();if(id==='holidays')renderHolidaysPage();if(id==='cinema')renderCinemaPage();if(id==='habits')renderHabitsPage();if(id==='ideas')renderIdeasPage();if(id==='pups')renderPupsPage();if(id==='recipes')renderRecipesPage();if(id==='packing')renderPackingPage();if(id==='bytype')renderKanban();if(id==='finance')renderFinancePage();if(id==='guide')renderGuidePage();if(id==='videos'){if(!_vidPageInit&&_prevPg!=='videos'){_vidView='dashboard';localStorage.setItem('_vidView','dashboard');}_vidPageInit=false;renderVideosPage();}if(id==='overview'){renderShopOv();renderRecOv();renderWkCal();if(document.getElementById('tbGrid'))renderDayTB();}else{const _tbSc=document.getElementById('tbScroll');if(_tbSc)_tbSc._scrollDay=null;}
...
// ══════════════════════════════════════════════════════════
const THEMES={
  // Warmed 2026-09-22 ("modern theme is a little too cold") — was a blue-gray neutral (B
  // channel highest in bg/orbs/swatch); shifted to a warm ivory/taupe neutral (R channel
  // highest instead) while keeping the same low saturation/lightness so it still reads as a
  // quiet neutral backdrop, not a tinted theme like Peach/Golden.
  modern:{name:'Modern',bg:'linear-gradient(135deg,#fffdfa 0%,#fefcf8 50%,#fffdfa 100%)',body:'#fefcf8',orbs:['radial-gradient(circle,rgba(198,182,160,.09),rgba(182,162,140,.04))','radial-gradient(circle,rgba(208,192,170,.08),rgba(192,172,150,.03))','radial-gradient(circle,rgba(193,177,157,.06),rgba(177,157,138,.02))','radial-gradient(circle,rgba(203,187,165,.06),rgba(187,165,145,.02))','radial-gradient(circle,rgba(198,182,161,.05),rgba(182,161,143,.02))'],swatch:'linear-gradient(135deg,#eae3d8,#f7f3e8,#efe8dc)'},
...
function getOvRecurring(){
  ...
  // today" path exists for this case, so it's included here to get one (same-week, so rollover
  // does a plain date pin — no this-time/all-future choice needed, see rolloverOverdue).
  ...
  // as that block (shared _wrWeekMisses() helper, overview.js) so the two can never disagree.
  // Tagged _wrWeekMiss so rolloverOverdue routes it through wrMoveToThisWeek (proper move-override
  // bookkeeping) instead of the same-week date-pin path above.
  _wrWeekMisses().forEach(({r,pwk})=>{
    out.push({_ruleId:r.id,_recId:r.id,_wkKey:pwk,due_date:pwk,done:false,name:r.name,_wrWeekMiss:true});
  });
  return out;
}
...
// Single self-contained "this occurrence only" move for a non-WR recurring task overdue from a
// past week — CARRY pattern (stays keyed under its original week, value updated to today), own
// undo/PATCH so it can run independently of whatever else rolloverOverdue is doing.
function _recMoveThisOccToToday(rec,wkKey){
  if(!rec._dateOverrides)rec._dateOverrides={};
  const prev=rec._dateOverrides[wkKey];
  const today=tod();
  rec._dateOverrides[wkKey]=today;
  save();renderAll();
...
// Same-week items skip this entirely (see the split in rolloverOverdue) since a same-week nudge
// isn't a schedule decision. Shares _wrScopePrompt with the per-row arrow menu (overview.js) so
// wording/order can't drift between the two entry points.
...
async function rolloverOverdue(clickEvent){
  ...
  // A past-week miss is a real schedule decision — prompt this-time/same-day/change-day for each
  // one (sequentially, see _rolloverPromptQueue), even in this bulk sweep. Same-week items (WR
  // pinned-this-week, or a non-WR task due earlier this week) are just a same-week date nudge, no
  // schedule implication, so those move silently along with the rest of the bulk sweep below.
...
document.addEventListener('keydown',e=>{
  ...
  if(e.key==='n'){if(activePg==='cinema')return;const _vp=document.getElementById('vidOvPanel');if(_vp&&_vp.style.display==='block')return;if(typeof _vidOvAllOpen!=='undefined'&&_vidOvAllOpen)return;e.preventDefault();openQA('today',null,d2s(getDayDate(dayOff)));}
