...
- **Selection persists across week navigation**: selection IDs are week-independent (`wrrule-virt-{id}`, `rec-virt-{id}`, `wrec-{id}` carry no wk_key), and week nav never clears `selectedTasks` — so a selected weekly-reset/recurring task stays highlighted in EVERY week it appears (select "Couch", arrow forward, it shows selected wherever present). The nav functions (`_goToWeek`, `shiftDay` cross-week, `goToday`) re-run `applySelHighlight` via rAF after re-render (renderAll does this too; the per-view nav paths must do it explicitly or the highlight silently drops while the data persists).
