...
- **Selection persists across week navigation**: selection IDs are week-independent (`wrrule-virt-{id}`, `rec-virt-{id}`, `wrec-{id}` carry no wk_key), and week nav never clears `selectedTasks` — so a selected weekly-reset/recurring task stays highlighted in EVERY week it appears (select "Couch", arrow forward, it shows selected wherever present). The nav functions (`_goToWeek`, `shiftDay` cross-week, `goToday`) call `applySelHighlight()` **synchronously** at the end (the week renders are synchronous, so the rows already exist). Do NOT use `requestAnimationFrame` here — rAF is frame-throttled (won't fire in a backgrounded tab, can race a re-render), which left the highlight silently unapplied even though `selectedTasks` was correct. Verified live 2026-06-26.
