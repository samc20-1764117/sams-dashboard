...
- **Pup Skills Highlight** (`#pupSkillsHighlight`): above `#dailyHabitsSection`, below `#todList`. Overview only. Shows `st.pup_skills` where `focus===true` and `stage!=='Mastered'`. Done-this-week state: `_trainedWk[wkKey]` (local only, not synced to server). `togPupSkillTrained` saves via `save()`. Sort: undone first, then Mochi→Sunny. Checkbox = trained this week (NOT mastered). Dblclick → `openPupEditModal(id)`. Title button → `showPage('pups')`. `+` button → `openPupAddModal()`. `savePupModal` calls `renderPupSkillsHighlight()` after add/edit. Styling: liquid glass container (`margin:10px 14px`), no count indicator. Header button matches WR grey style (no purple). Skill text: `font-size:11px;font-weight:400;color:var(--muted)`. Checkbox: soft outward pup-colored glow (`0 0 4px 2px`). Hover tooltip (`showPupSkillTip`/`hidePupSkillTip`): skill name bold + pup letter (M/S) right-aligned in pup color; next_step value medium/lighter; notes lightest (`var(--subtle)`). Timeblock labels at 8am/4pm are darker+bold (`rgba(45,40,85,.95)`, weight 800) on Mon–Fri only.
