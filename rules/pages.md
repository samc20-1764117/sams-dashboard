...
- **Move to Ideas clears post_date**: `_vidOvDemote`, `_vidOvBulkDemote`, and arrow-key status changes all clear `post_date` (and children's) when moving to `idea` status. Prevents idea videos from appearing on monthly calendar. Undo restores original `post_date`.
...
- **Card headers** (`.ct`): all use the base style — 9px, UPPERCASE, `letter-spacing:.08em`. The Shopping / "N ideas" / Archived Ideas headers previously had one-off inline `text-transform:none;letter-spacing:0;font-size:10px` overrides; those were removed for consistency. Do NOT re-add per-header case/size overrides. Shopping header cart icon is SVG, not emoji.
