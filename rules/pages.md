...
- **Videos on overview**: video camera button in `wkc-nav` (`#vidOvBtn`). Click toggles `#vidOvPanel` — slides in from left over the today/timeblock card, showing up_next B videos with children, stage dots, progress %, and `+` buttons to add smalls (mimics videos page). Panel stays open after drag. Assigned videos show amber dot + dimmed. "Videos" header text links to videos page. Drag onto day column → `_vidAssignToDay(vidId,ds)` stores in `localStorage._vidDayMap` (undoable). Video appears as amber chip (`rgba(251,191,36,.18)`) with video icon. Check off → `_vidCompleteFromOv`: marks stages done except tab/up, prompts post date near checkbox, creates tab/up task. ✕ → `_vidUnassignDay` (undoable). Dblclick → `openVidEdit` (stays on overview, doesn't navigate to videos page).
