...
- **Videos on overview**: video camera button in `wkc-nav` (`#vidOvBtn`). Click opens `#vidOvMenu` dropdown listing Big videos with status `up_next`/`in_progress`. Drag from menu onto day column → `_vidAssignToDay(vidId,ds)` stores in `localStorage._vidDayMap`. Video appears as pink chip (`gc('videos')`) with video icon prefix. Check off → `_vidCompleteFromOv`: marks all steps done + children + sets status `published`. ✕ → `_vidUnassignDay`: removes from calendar only. Dblclick → `openVidEdit`. No posted/duration fields in edit modal (hidden inputs preserve data).
