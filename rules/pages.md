...
- **Book layout**: `.rec-book` two-panel grid (`320px 1fr`), `border-radius:12px`, no border/seam between panels — seamless glassy design. Left: scrollable recipe list. Right: `#recDetailPanel` always-visible detail view. Active recipe item styled as tab opening into detail (border-radius left only, matching right-panel background, no right border). Page padding matches all other pages (`clamp(12px,3vw,56px)`). Wrap height: `calc(100vh - 102px)`.
- **Panel styling**: frosted glass (`backdrop-filter:blur(12px)`), subtle border, rounded corners (left panel: left corners, right panel: right corners). Title flourish gradient under recipe name, stacked meta labels (Meal/Cuisine/Time/Serves as label-above-value). Source link field (`r.source`) at bottom-right.
...
- **Ingredients** (`_detailIngs` array, modal-style): in-memory array rendered as always-visible input rows (`_diRender`). No click-to-edit — inputs always shown. Tab: amt → name → next amt → name. Tab from last empty row removes it and focuses instructions (cursor at end). `_diAdd` adds new row focusing amt. `_diSave` persists on blur-away. Drag-reorder from anywhere on row (inputs stop propagation). Delete button (`di-del-btn`) has `tabindex="-1"`. `_diKey` handles Tab/Enter/Backspace + autocomplete arrow navigation. Autocomplete: `_COMMON_INGS` + all used ingredient names. Arrow up/down to navigate, Enter to pick → focuses next row's amt.
