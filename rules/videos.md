...
- **Overview pop-up floating panels** (Calendar/All-videos/Analytics — `_vidOvRenderCal`/`_vidOvRenderAll`/`_vidOvRenderAnalyticsPanel`, overview.js): each is a `position:fixed` div sized via `_vidOvContentRect()` (overview.js), which returns `#main`'s rect inset by `#page-overview`'s own horizontal padding — NOT `.overview-cols` (that has `max-width:1600px` and centers itself once the viewport is wider, so its own bounds silently drift from the page's actual content edges) and NOT raw `#main` alone (no max-width, but ignores the page's padding and overshoots). Colors (`_bg`/`_dk()`-branched) are recomputed and reapplied on EVERY call, not just at creation — these panels used to only get colored once, so toggling dark mode while one was already open left it stuck on the old theme; `toggleDark()` (features.js) now explicitly re-renders whichever of the three is open. Edit-Title mode (`_vidOvToggleTitleMode`) extends the main `#vidOvPanel` itself the same way (explicit `left`/`width` in page coordinates via `_vidOvContentRect()`, not a `right` offset relative to the card — the card can sit inside centered `.overview-cols` margin, so a card-relative offset silently inherits that gap). Calendar/All-videos are still separate floating panels (not extend-in-place like Title mode) — reworking that to slide out from the list instead of floating on top is a known follow-up, not yet done.
- **Overview pop-up child numbering** (`_vidOvMenuItem` in overview.js, small videos nested under a big one): each gets `N.` — right-aligned in a fixed 16px column so periods always land on the same pixel regardless of digit count (verified via `Range.getBoundingClientRect()` on the period character itself, not just the span box). Requires `min-width:0` on that span — flex items default to `min-width:auto`, which silently lets a wider 2-digit number grow its box past the declared `width` and break the alignment; this bit us for several rounds before the `min-width:0` fix. Digits use `font-family:Menlo,Consolas,monospace` (tabular + a non-slashed zero — `ui-monospace`/`SF Mono` render a slashed zero on Mac by default, which was also rejected). The topic/title/comment columns' own leading spacer is tuned to this exact number-column width (currently 13px) — if the number column's width or margin changes, re-measure and retune the spacer empirically, don't recompute it by hand (the flex math involves non-1:1 redistribution between fixed and `flex:1` siblings that's easy to get wrong analytically).
