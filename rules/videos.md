...
- `Cmd+Right` — demote status: up_next → in_progress → idea. L videos moving to idea auto-clear `big_video_id` (ungroup). Works with multi-select.
- `Cmd+Left` — promote status: idea → in_progress → up_next (caps at up_next)
