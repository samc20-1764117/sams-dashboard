...
- **Videos pop-up focus week** (`_vidOvFocusWk`): highlights a video/step only while it has an INCOMPLETE instance in `getWkKey(wkOff)` — not whether the whole video/group is done (2026-08-25 fix; presence alone used to count, so a fully-checked-off step kept the yellow highlight until the whole group was published). Per-step check reuses `_vidStepComputeDone(vidId,step,ds,mapEntry)` (the same helper the overdue banner uses) against every day a step occupies this week (primary `ds` plus any `extraDays`). Finishing this week's steps drops the highlight even if other weeks/the group aren't done; scheduling something new for tomorrow brings it right back. Pop-up re-renders on every week change so the highlight tracks the currently-shown week.
- **Overview pop-up floating panels** (All-videos/Analytics — `_vidOvRenderAll`/`_vidOvRenderAnalyticsPanel`, overview.js): each is a `position:fixed` div sized via `_vidOvContentRect()` (overview.js), which returns `#main`'s rect inset by `#page-overview`'s own horizontal padding — NOT `.overview-cols` (that has `max-width:1600px` and centers itself once the viewport is wider, so its own bounds silently drift from the page's actual content edges) and NOT raw `#main` alone (no max-width, but ignores the page's padding and overshoots). Colors (`_bg`/`_dk()`-branched) are recomputed and reapplied on EVERY call, not just at creation — these panels used to only get colored once, so toggling dark mode while one was already open left it stuck on the old theme; `toggleDark()` (features.js) now explicitly re-renders whichever is open. Edit-Title mode (`_vidOvToggleTitleMode`) extends the main `#vidOvPanel` itself the same way (explicit `left`/`width` in page coordinates via `_vidOvContentRect()`, not a `right` offset relative to the card — the card can sit inside centered `.overview-cols` margin, so a card-relative offset silently inherits that gap).
- **Calendar now extends the panel in-place too, like Title mode** (`_vidOvToggleCal`/`_vidOvRenderCal`, overview.js, 2026-08-26 rework — was previously a `position:fixed` floating panel like All-videos/Analytics above, per user request to sit beside the list instead of on top of it): `_vidOvToggleCal` grows `#vidOvPanel` via `panel.style.right=(-ext)+'px'` where `ext` reaches `_vidOvContentRect()`'s right edge (same overshoot-safe content rect as Title mode, floored at 300px), and stashes the panel's PRE-extension width in `panel.dataset.calListW` so `_renderVidOvMenu` can keep the video list at that fixed width while the calendar column (`#vidOvCalEmbed`, injected as a flex sibling) fills the rest — list never reflows when the calendar opens/closes. `_vidOvRenderCal` now renders into that embed div (found via `getElementById('vidOvCalEmbed')`, no-ops if missing) instead of creating/positioning its own panel; `_vidCalHighlightChip` targets the same id. `_renderVidOvMenu` re-invokes `_vidOvRenderCal` at its own tail whenever `_vidCalOpen`, since every call to `_renderVidOvMenu` (there are dozens of call sites across the codebase) fully replaces `#vidOvPanel`'s innerHTML and would otherwise wipe the embed. **Gotcha**: the calendar's "scroll to current month" logic must scroll `#vidCalScroll` directly (`scrollEl.scrollTop += target.rect.top - scrollEl.rect.top`) rather than `target.scrollIntoView(...)` — scrollIntoView walks every scrollable ancestor, and now that the calendar lives inside `#main` (a real scroll container) instead of a body-appended fixed panel, it would drag `#main`'s own scroll position along with it, visibly yanking the whole panel out of view on open. If you ever touch that scroll-to-month code again, don't reintroduce scrollIntoView there. **Known pre-existing bug, not touched by this rework**: the prev/next month arrow buttons only mutate `_vidCalMonth`/`_vidCalYear` and re-render — `_vidOvRenderCal` never reads either variable, it always shows the full continuous month range from earliest video to next-year-end and re-scrolls to the REAL current month on every render, so the arrows currently do nothing visible. Confirmed via git diff that this predates the 2026-08-26 embed rework (identical onclick handlers before/after) — fix separately if asked, don't assume it's a regression from the rework.
- **Overview pop-up child numbering** (`_vidOvMenuItem` in overview.js, small videos nested under a big one): each gets `N.` — right-aligned in a fixed 16-20px column so periods always land on the same pixel regardless of digit count (verified via `Range.getBoundingClientRect()` on the period character itself, not just the span box). Requires `min-width:0` on that span — flex items default to `min-width:auto`, which silently lets a wider 2-digit number grow its box past the declared `width` and break the alignment; this bit us for several rounds before the `min-width:0` fix. Digits use `font-family:Menlo,Consolas,monospace` (tabular + a non-slashed zero — `ui-monospace`/`SF Mono` render a slashed zero on Mac by default, which was also rejected). The number span carries both `margin-left:-3px` and `margin-right:-3px` (2026-08-26) — the row's `└` angle icon and the number+topic block sit in one `gap:5px` flex row, so a negative margin on ONLY the number's left side pulls it (and the topic span after it, via the unchanged right-side gap) a few px closer to the icon without moving the icon itself; don't add the same negative margin to the icon's own div, that would just double the shift. The topic/title/comment columns' own leading spacer is tuned to this exact number-column width (currently 13px) — if the number column's width or margin changes, re-measure and retune the spacer empirically, don't recompute it by hand (the flex math involves non-1:1 redistribution between fixed and `flex:1` siblings that's easy to get wrong analytically).
- **Big-video row has no small-video count badge or top-level "add small" circle-button pairing anymore** (`_vidOvMenuItem`, 2026-08-26 — user explicitly asked to remove the `· N` count badge that used to sit after the topic name; the `+` add-small-video button to the LEFT of the topic was removed in the same pass then explicitly asked back a message later — it's back, the count badge is not). If asked to touch this row again, don't reintroduce the count badge without being asked.
- **Deleting or demoting a video to idea removes its stage tasks too** (`_vidStepCleanupForVideo`/`_vidStepCleanupRestore`, overview.js, 2026-08-26): previously `delVideo` (videos.js)/`_vidOvBulkDelete`/`_vidOvDemote`/`_vidOvBulkDemote` (overview.js) only touched the video record itself (`is_deleted`/`status`) — any Build/VO/Cut/Th/Des stage already scheduled via `_vidStepDayMap` or a real `st.blocks` timeblock entry (`_vidStepVid`/`_vidStepName`) kept rendering in the weekly view/timeblock grid forever, since nothing ever cleared those. `_vidStepCleanupForVideo(vidId)` deletes every `_vidStepDayMap` key for that id and every matching `st.blocks` entry (via `sbDeleteBlock`), returning a snapshot; `_vidStepCleanupRestore(snapshot)` re-adds them. Called (and its result stashed for `pushUndo`) from all four call sites above, including the child-video cascade inside `_vidOvDemote`/`_vidOvBulkDemote` when a Big video's Littles get demoted along with it. Any NEW code path that deletes or idea-demotes a video must call this too, or its stage tasks will silently outlive it.
...
- **Standalone videos**: videos without `big_video_id` are standalone. **As of 2026-09-11, standalone L videos are fully first-class** — they can sit in `in_progress`/`up_next` same as grouped ones. The old render-time guard that silently forced any parentless L back to `idea` the moment it touched an active status (`renderVideosPage()`, plus matching checks in `_vidDashDrop` and `saveVidModal`) was removed at explicit user request — do NOT reintroduce it. `renderVideosPage()` still clears a *dangling* `big_video_id` (parent got deleted) but no longer touches status when doing so. On the Current tab specifically, `_vidGroupDrop` (drag-to-nest into a B's group) only accepts a non-B video as the dragged item — it silently no-ops if you drag a B onto another B here. That's unchanged; Big-onto-Big nesting (demote-with-orphan-promotion) is a separate, newer system that only exists in the Overview popup/toolbox — see `rules/pages.md`'s "All Videos toolbox" / "Nest by dragging" entries.
...
- **`saveVidModal`'s "place at bottom of group" block only runs on an actual group CHANGE now** (fixed 2026-09-22): it used to reassign `vid_order=maxOrder+1` any time `data.big_video_id` was truthy, with no check for whether the video was already in that group — so re-saving ANY grouped L video's edit (rename, status, whatever) recomputed its order against its own siblings (itself included in the max) and bottomed it out, every single time. Symptom: "editing a small video's name moves it to the bottom of In Progress/Up Next/Ideas." Now compares `data.big_video_id` against the pre-edit video's own `big_video_id` (looked up via `_vidEditId` while still in edit mode) and only recomputes the bottom-of-group order when they differ (or on add, where there's no existing video to compare against) — leaving `data.vid_order` unset on a same-group edit so `Object.assign(v,data)` and the PATCH simply leave the existing order alone. If a similar "auto-place" mutation is ever added elsewhere (drag/drop, bulk edit, etc.), guard it the same way — only touch order on an actual group/position change, never unconditionally on every save.
...
- **Client safeguards**: `_ytFetched`/`_ytAnalyticsFetched` global flags — set `true` before fetch, only reset on full page reload. One fetch per page load, period. The actual fetch calls live in shared `_ytEnsureFetch()`/`_ytEnsureAnalyticsFetch()` (videos.js, top of file) so BOTH the Videos page render AND the Overview page's video pop-up analytics panel (`_vidOvRenderAnalyticsPanel`, overview.js) can request the data — whichever calls first fetches, the other sees the flag already set and no-ops. Never add a fetch call anywhere else; always route through these two functions, or the one-fetch-per-session guarantee breaks. Also caches to `localStorage._ytCache`/`_ytAnalyticsCache` as offline fallback.
