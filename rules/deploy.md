...
**A PARTIAL bump is worse than no bump at all — it causes an infinite reload loop, not just staleness** (hit 2026-09-15, user-reported as "the dashboard keeps glitching"): `version.json`'s `v` and the `<script>` `?v=` params got bumped to a new value, but the separate `window._BUILD='...'` literal a few lines above them in `index.html` was missed (easy to miss — it's a bare JS string assignment, not a `?v=` on a tag). Since `index.html`'s own version-check compares `BUILD` to `version.json` on every load and force-`location.reload()`s on any mismatch, and the reloaded page is still the same stale static HTML with the old `_BUILD` baked in, the mismatch can never resolve — it reloads forever, hundreds of times a second in the network log. Diagnose via the browser's network tab / `read_network_requests`: repeating `/`, `version.json`, `core.js`, etc. a few hundred ms apart. **Always grep `window._BUILD=` specifically (not just `?v=`) and confirm it matches the new `version.json` value before ending the turn** — a full skip of the bump just leaves things stale (bad but inert); bumping some but not all three actively breaks the page for every visitor.
