...
**Shift schedule (all future)** (`wrCtxShiftSchedule`): shifts `starting_date` ±7 days so ALL future occurrences realign (always happens). The current-week instance is relocated to the adjacent week (skip override + `_dateOverrides` pin + WR-rule move override) ONLY IF `_appearsThisWk` — i.e. the task is pinned this week OR naturally due on the OLD schedule (`isWRRuleDueThisWeek`/`getRecurringWeekTasks`). For an off-cycle week (e.g. a biweekly task on a non-due week), it would otherwise skip/move a non-existent instance and leave a phantom (made "move all future" look broken). Override-tracking vars (`_prevOv/_ovRealId/_movedOv`) are function-scoped so the undo closure can see them.
