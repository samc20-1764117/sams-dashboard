...
- **Month heatmap** (`#rt-heat`, `renderRtHeatmap()`): inline calendar above the two columns (not a separate view/button) — every WR + non-WR occurrence for the displayed month, color-coded by cadence via small dots (`--home-d`=weekly, `--soc-d`=biweekly, `--buy-d`=monthly, `--ov-d`=other/quarterly+). Reuses `getRecurringWeekTasks`/`isWRRuleDueThisWeek` exactly like the "Next" column, so it can't drift out of sync with the lists below. Month nav via `_rtHeatShift(±1)`/`_rtHeatToday()`, local state (`_rtHeatMonth/_rtHeatYear`, null = current month).
