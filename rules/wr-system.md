...
Three-column layout (`#rt-outer`, height `52vh`): `#rt-col-wr` (WR rules, weekly cadence only) | `#rt-col-wr-rest` (WR rules, biweekly/monthly/other) | `#rt-col-sch` (all non-WR recurring, `renderRtGroup`, all 4 cadences stacked). The WR split across two columns is purely a container-placement thing — `renderRtWrGroup('rt-wr-'+cad, ...)` still targets `#rt-wr-weekly`/`#rt-wr-biweekly`/`#rt-wr-monthly`/`#rt-wr-other` exactly as before; those divs just live in different parent columns now (`#rt-wr-weekly` in `#rt-col-wr`, the other three in `#rt-col-wr-rest`). Don't add JS branching for this — if the split ever needs to change again, move the container divs, not the render function. Headers are Title Case, not uppercase (deliberate — don't reintroduce `text-transform:uppercase`).
...
- **Month heatmap** (`#rt-heat`, `renderRtHeatmap()`): compact inline calendar BELOW the two columns (not a separate view/button, not above) — every WR + non-WR occurrence for the displayed month, color-coded by cadence via small dots (`--home-d`=weekly, `--soc-d`=biweekly, `--buy-d`=monthly, `--ov-d`=other/quarterly+). No separate legend — the same dot+color is prepended to each cadence group's header in both `renderRtWrGroup`/`renderRtGroup` (`.rt-heat-dot.cad-{cadence}`), so the lists double as the legend. `#rt-outer` (the two columns) is a fixed `height:70vh`, not `100vh`-flex-constrained; the heatmap sits after it in normal flow and `#main`'s existing `overflow-y:auto` handles page-level scrolling if the two together exceed viewport height — no extra scroll wrapper needed. Its own grid also caps at `max-height:150px;overflow-y:auto` as a belt-and-suspenders safeguard. Reuses `getRecurringWeekTasks`/`isWRRuleDueThisWeek` exactly like the "Next" column, so it can't drift out of sync with the lists above it. Month nav via `_rtHeatShift(±1)`/`_rtHeatToday()`, local state (`_rtHeatMonth/_rtHeatYear`, null = current month).
