...
**Shift schedule (all future)** (`wrCtxShiftSchedule`): "All future → Next/Prev". Just shifts `starting_date` by `delta` days (±7) — that alone moves the next occurrence AND all future ones a week, correct for every interval cadence (biweekly = parity flip, monthly = day-of-month, quarterly = anchor week). Falls back to the source week (`_wrCtxWkKey`) if `starting_date` is null. Re-renders + shows a confirmation toast + undoable. Does NOT touch `_dateOverrides` or write move overrides (the old version's skip + move-override dance failed silently and made it look like "nothing happens"). The current occurrence follows the new cadence naturally.
