...
Two-column layout: `#rt-col-wr` (WR rules, `renderRtWrGroup`) | `#rt-col-sch` (non-WR recurring, `renderRtGroup`) — each stacked into Weekly/Biweekly/Monthly/Other cadence tables (`.rt-tbl`). Headers are Title Case, not uppercase (deliberate — don't reintroduce `text-transform:uppercase`).
