...
**Shift schedule (all future)** (`wrCtxShiftSchedule`): "All future → Next/Prev". Shifts `starting_date` by `delta` days (±7) — moves the context week's occurrence AND every future one a week (biweekly = parity flip, monthly = day-of-month, quarterly = anchor week). ALSO clears anything pinning the rule to the CURRENT week so the shift is visible: deletes `_dateOverrides[wkKey]` and removes stale `move` overrides (`wk_key===wkKey` OR `moved_to_wk_key===wkKey`).
- **Past is frozen** (scope = "this week + all later"): shifting `starting_date` reparents the WHOLE series, which would otherwise rewrite history (parity flips both ways → past occurrences move/appear/disappear). Before shifting, for each week in the prior 13 (interval cadences only) with no existing override, the OLD schedule is nailed in place: pin the week's date if it occurs now, else `_dateOverrides[wk]='__skip__'`. 13 weeks covers everything that renders (~4-wk lookback). Both pins and `__skip__` are honored by the predicates: `getRecurringWeekTasks` (`hasPin` bypasses start-date + parity gates; `__skip__` blocks) and `isWRRuleDueThisWeek` (non-skip pin → force-show, `__skip__` → force-hide). Undo deletes the frozen keys, restores the current-week pin, re-POSTs removed move overrides, restores `starting_date`.
