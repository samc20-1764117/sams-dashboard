...
- **Weekly cal day columns** (`_wkcDragTid`/`_wkcDragSrcDs`, core.js): same idea, delegated on `#wkcCols` (set up once in `renderWkCal`, guarded by `cols._reorderSetup`) since chips there use `dataset.tid` not an `id="ti-..."` attribute — tracks the dragged chip's tid *and* which column (`ds`) it started in. Each column's own `dragover`/`drop` handlers check `_wkcDragTid&&_wkcDragSrcDs===ds` first (placeholder = `.wkc-col-ph`) and return early on a match, before any of the existing cross-day-drop branches or edge-zone/travel-drag-select logic runs.
