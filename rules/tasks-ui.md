...
- **Vidstep copy/paste**: Cmd+C copies vidstep tasks (`_isVidStep` flag, regex `^vidstep-(\d+)-(step_\w+)` handles both ID formats). Cmd+V on Build/VO/Cut calls `_vidStepAddBlock` (duplicate, not move). Th/Des steps can't be pasted (toast warning). Target day from `_pasteColDates` (weekly column click).
