...
- **Vidstep overdue**: Auto-moves in `renderToday` when `dayOff===0`: (1) daymap entries with `val.ds < today` get `ds` set to today, (2) ALL undone vidstep blocks on past days get moved to today. Same behavior as regular video auto-move. After auto-move, `_vidStepTasksForDayWithOverdue(todayDs)` returns them as normal today items. `_vidStepOvCount` also scans past-day blocks as fallback.
