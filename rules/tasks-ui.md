...
- **Vidstep operations — today list vs weekly chips**:
  ...
  - **Weekly chips**: check/delete pass `forDay=t.due_date` → only affect that day's blocks. Drag uses `vidstep::vid::step::sourceDay` — drop moves source day's blocks AND always updates daymap primary to target day (even if daymap previously pointed elsewhere).
