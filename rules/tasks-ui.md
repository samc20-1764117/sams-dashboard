...
- **Vidstep overdue**: Auto-moves overdue daymap entries to today (same as regular videos) in `renderToday` when `dayOff===0` — moves blocks to today. `_vidStepTasksForDayWithOverdue(todayDs)` includes daymap entries where `val.ds <= todayDs` AND scans TB blocks on past days not covered by daymap. `_vidStepOvCount` also scans past-day blocks.
