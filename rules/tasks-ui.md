...
- `updateOvBanner()` called from `renderToday()`. Banner text: `X Overdue` + `Move to today` button. Always silent/no-popup for every overdue type including recurring/WR, regardless of how many are overdue at once — "this occurrence only" semantics throughout (WR week-misses go through `wrMoveToThisWeek(id,wkKey,false)` for proper move-override bookkeeping; everything else is a plain date pin). Fine-grained control lives on each row instead (below), not the bulk button.
- **Per-row "move to today" arrow** (`_dlblOvArrow`, overview.js): every overdue Today-list row (`tRow`, `tRowTodayVirt`, `tRowShopVirt`, `tRowVidVirt`, `tRowPupSess` — NOT vidsteps, which intentionally never auto-move) gets a small ▾ after its day letter. Non-recurring types (`_taskMoveToToday`/`_shopMoveToToday`/`_pupMoveToToday`/`_vidMoveToToday`) move that one item directly, no menu — there's no schedule question for a one-off task. Recurring/WR route through `_ovRowMoveClick`, which checks whether the occurrence's `_wkKey` is the current week: same-week (a task/WR-pin overdue from earlier *this* week) moves directly, no menu; a genuine past-week miss opens `showWrScopePicker` with **3** options — This time only / All future (same day) / All future (set day). "Same day" re-anchors the schedule to resume now without changing which weekday it falls on (`_recMoveAllFuture(...,keepDay=true)` / `wrMoveToThisWeek(...,allFuture,setDow=false)`); "set day" also changes the weekday to match today (`_nthWeekdayOfMonth` recomputes monthly nth-weekday rules). This arrow/menu is Today-list-only — the Weekly Reset container's own red overdue rows deliberately have no move action (see `rules/wr-system.md`).
