...
- **Vidstep done consistency (CRITICAL)**: ALL overdue vidstep checks MUST use `_vidStepComputeDone()` — never rely on `val.done` alone. `val.done` only covers Th/Des (daymap flag). Build/VO/Cut done state lives on timeblock blocks (`bl._done`), only accessible via `_vidStepComputeDone`. This applies to: `_vidStepOvCount` (overview.js), `updateOvBanner` (features.js), and `rolloverOverdue` (features.js). Using `val.done` instead causes phantom overdue counts with nothing in the list.
