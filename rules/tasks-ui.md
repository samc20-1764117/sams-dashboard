...
- **Per-row "move to today" arrow** (`_dlblOvArrow`, overview.js): every overdue Today-list row (`tRow`, `tRowTodayVirt`, `tRowShopVirt`, `tRowVidVirt`, `tRowPupSess` — NOT vidsteps, which intentionally never auto-move) gets a small → after its day letter, shifted to `right:24px` (vs. the base `right:9px`) so it clears the always-visible (not hover-only — see `.ti.ov-row .delbtn` in styles.css) delete button. Non-recurring types (`_taskMoveToToday`/`_shopMoveToToday`/`_pupMoveToToday`/`_vidMoveToToday`) move that one item directly, no menu. Recurring/WR route through `_ovRowMoveClick`: same-week moves directly, past-week opens the same 3-option prompt as the bulk queue above — both call the shared `_wrScopePrompt(e,onThisTime,onSameDay,onChangeDay)` (overview.js) so wording/order can't drift between the two entry points. Options display top-to-bottom as **This time only / Same day / Change day to today** — `#wrScopePicker`'s markup renders remove-slot first/this-slot second/all-slot third, so `_wrScopePrompt` deliberately maps this-time→removeLabel, same-day→thisLabel, change-day→allLabel to get that order; don't "simplify" that mapping. "Same day" re-anchors the schedule to resume now without changing which weekday it falls on (`_recMoveAllFuture(...,keepDay=true)` / `wrMoveToThisWeek(...,allFuture,setDow=false)`); "change day" also changes the weekday to match today (`_nthWeekdayOfMonth` recomputes monthly nth-weekday rules). This arrow/menu is Today-list-only — the Weekly Reset container's own red overdue rows deliberately have no move action (see `rules/wr-system.md`).
