...
- **Vidstep TB copy/paste**: Cmd+C on a vidstep TB block copies `_vidStepVid` + `_vidStepName`. Cmd+V creates a new independent block on the same day (separate TB instance). Daymap entry auto-created if missing. This is distinct from task-level Cmd+V paste which uses `_vidStepAssignExtraDay`.
...
- **Vidstep reconstruction**: `_vidStepReconstructBlocks()` — first deletes ghost vidstep blocks with `sm < HOURS[0]*60` (invisible above TB column). For multi-day Build/VO/Cut, only creates daymap entry if none exists (does NOT overwrite existing to today, preserving overdue). For single-day Th/Des, always syncs daymap to today.
