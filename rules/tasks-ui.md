...
- **Vidstep overdue**: `_vidStepTasksForDayWithOverdue(todayDs)` includes daymap entries where `val.ds <= todayDs` AND scans TB blocks on past days not covered by daymap. Does NOT auto-move daymap. `_vidStepOvCount` also scans past-day blocks.
