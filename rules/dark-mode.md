...
## Pages Completed
- **Overview** (overview.js, styles.css): full dark mode — cards, timeblocks, week cal, kanban, shopping, WR, quick links, donut, month cal, meals divider, quick-add popup (`.qa-popup`, category select trigger/dropdown), weekly-header selected-day circle text, Edit Task modal (Notes/Time Block), WR/recurring modal Notes fields, Daily Habit popup, Birthday present popup, all context menus + tooltips (see audit note in Patterns above).
