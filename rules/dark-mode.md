...
- **Inline JS-set styles always beat `body.dark` CSS** (inline specificity beats any stylesheet selector). A hardcoded `style="background:rgba(255,255,255,.8)"` written in JS template strings (quick-add popup fields, category-select trigger) will NOT respond to dark mode no matter what CSS you add. Fix by either wrapping in a `_dk()`/`_isDk()` ternary at render time, or (preferred for anything already covered by a class-based CSS rule) just deleting the inline background and letting the class rule + its `body.dark` override cascade normally.
...
## Pages Completed
- **Overview** (overview.js, styles.css): full dark mode — cards, timeblocks, week cal, kanban, shopping, WR, quick links, donut, month cal, meals divider, quick-add popup (`.qa-popup`, category select trigger/dropdown), weekly-header selected-day circle text.
